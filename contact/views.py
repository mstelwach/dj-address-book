...
class ContactUpdateView(LoginRequiredMixin, SuccessMessageMixin, UpdateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        phone_table = PhoneTable(data=qs_phone, template_name="django_tables2/bootstrap4.html")
        email_table = EmailTable(data=qs_email, template_name="django_tables2/bootstrap4.html")
        RequestConfig(self.request, paginate={'per_page': self.paginate_by}).configure(phone_table)
        RequestConfig(self.request, paginate={'per_page': self.paginate_by}).configure(email_table)
